...
kmeans_model_name = sys.argv[1]
file_test = os.path.join(dirname,'labelled_dataset.csv')

fig_name = 'confusion_matrix_'+ kmeans_model_name.split('.')[0] + '.png'
supervised_model = 'Supervised_svm_'+ kmeans_model_name.split('.')[0] + '.joblib'
...
clusters = load(kmeans_model_name)
feature = clusters.transform(X_super)
feature = np.reciprocal(feature)
feature = feature/feature.sum(axis=1)[:,None]
for row in range(0, len(feature)):
    for element in range(0, len(feature[row])):
        # print(len(row), element)
        if np.isnan(feature[row][element]):
            feature[row][element] = float(1)
            print("nan replaced")
